...
- **Cache**: Cache for Google Sheets data (Default cache is 10 minutes)
- **Session**: User session management

### Data Flow

1. **Read Operations**: Cached in D1, background refresh every xx seconds (default cache time is 10 minutes)
    - If cache miss, fetch from Google Sheets and update cache
    - If the cache is expired, call Cloudflare background task and return last cache.
