...
export class ConfigRepository {
  ...
  /**
   * Save user session
   * Sessions are stored as encrypted JSON in the config table
   *
   * @param sessionId - Unique session identifier
   * @param sessionData - Session data including userId, username, roles
   * @param expiresAt - Session expiration time
   */
  async saveSession(
    sessionId: string,
    sessionData: { userId: string; username: string; roles: string[] },
    expiresAt: Date
  ): Promise<void> {
  ...
  async getSession(
    ...
    const sessionJson = await this.getDecrypted(`session:${sessionId}`);
    if (!sessionJson) {
      return null;
    }
