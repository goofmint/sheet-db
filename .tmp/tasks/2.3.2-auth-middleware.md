...
app.get('/api/v1/sheets/:sheetName/data', (c) => {
  ...
  // ACLService でpublic_readフラグをチェック
  if (authContext.isAuthenticated) {
    // 認証済みユーザー向けの処理
    return c.json({ 
      sheet: sheetName,
      data: 'user accessible data'
    });
  }
  // 公開データのみ（public_read = trueの場合）
  return c.json({ 
    sheet: sheetName,
...
// 特定のロールが必要なシート操作
app.use('/api/v1/sheets/:sheetName/admin', auth(), requireRoles(['sheet_admin']));

app.put('/api/v1/sheets/:sheetName/admin/settings', (c) => {
  // sheet_adminロールを持つユーザーのみアクセス可能
  const sheetName = c.req.param('sheetName');
  return c.json({ message: `Updated settings for ${sheetName}` });
});
```
