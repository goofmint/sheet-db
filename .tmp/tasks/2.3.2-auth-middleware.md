...
// 特定のロールが必要なエンドポイント（例：シート管理操作）
app.use('/api/v1/sheets/:sheetName', auth(), requireRoles(['sheet_admin']));

app.delete('/api/v1/sheets/:sheetName', (c) => {
  // sheet_adminロールを持つユーザーのみアクセス可能
  const sheetName = c.req.param('sheetName');
