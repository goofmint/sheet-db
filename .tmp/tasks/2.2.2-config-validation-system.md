...
-- API endpoints configuration (JSON type)
UPDATE Config SET validation = '{\"required\":false,\"type\":\"json\",\"errorMessage\":\"API endpoints must be valid JSON object\"}' 
WHERE key = 'api.endpoints';

-- Theme configuration (JSON type)
UPDATE Config SET validation = '{\"required\":false,\"type\":\"json\",\"errorMessage\":\"Theme configuration must be valid JSON object\"}' 
WHERE key = 'app.theme_config';
```
...
export class JsonValidator extends BaseValidator {
  validate(value: string, rule: ValidationRule): ValidationResult {
    ...
    try {
      ...
      return this.createError(rule.errorMessage || 'Must be valid JSON format');
    }
    
    // Additional JSON schema validation could be added here
    // if rule.schema is provided
...
export class ConfigValidator {
  ...
  async validateField(key: string, value: string, allConfig: Record<string, string>, rules?: Record<string, ValidationRule>): Promise<ValidationResult> {
  ...
  async validateAll(config: Record<string, string>): Promise<ValidationResults> {
    ...
    for (const ruleKey of Object.keys(rules)) {
      ...
      const result = await this.validateField(ruleKey, value, config, rules);
      if (!result.valid) {
        results.valid = false;
    ...
    for (const [key, value] of Object.entries(config)) {
      if (!rules[key]) {
        const result = await this.validateField(key, value, config, rules);
        if (!result.valid) {
          results.valid = false;
